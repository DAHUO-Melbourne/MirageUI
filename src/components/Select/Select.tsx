...
export const Select:FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('mirage-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="mirage-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="mirage-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="mirage-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="mirage-tag" key={`tag-${index}`}>
                  {value}
