...
export const Tabs: FC<TabsProps> = (props) => {
  ...
  const navClass = classNames('mirage-tabs-nav', {
    'nav-line': type === 'line',
    'nav-card': type === 'card',
  })
  const renderNavLinks = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const classes = classNames('mirage-tabs-nav-item', {
  ...
  return (
    <div className={`mirage-tabs ${className}`}>
      <ul className={navClass}>
        {renderNavLinks()}
      </ul>
      <div className="mirage-tabs-content">
        {renderContent()}
      </div>
    </div>
  )
